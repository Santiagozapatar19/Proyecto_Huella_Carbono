...
export default function DataTable({ columns, data, onEdit, onDelete, loading }) {
  if (loading) {
    return (
      <div className="flex items-center justify-center h-40 text-carbon-800">
  ...
  if (!data?.length) {
    return (
      <div className="text-center py-12 text-carbon-800">
    ...
    <div className="overflow-x-auto rounded-lg border border-carbon-200">
      <table className="w-full text-sm">
        <thead className="bg-carbon-50 border-b border-carbon-200">
          <tr>
            {columns.map((col) => (
              <th key={col.key} className="text-left px-4 py-3 text-xs font-semibold text-carbon-800 uppercase tracking-wide">
                {col.label}
              </th>
            ))}
            {(onEdit || onDelete) && (
              <th className="text-right px-4 py-3 text-xs font-semibold text-carbon-800 uppercase tracking-wide">
                Acciones
              </th>
            )}
          </tr>
